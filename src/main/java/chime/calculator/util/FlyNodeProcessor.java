...
public class FlyNodeProcessor extends NodeProcessor {
    ...
    public int findPathOptions(PathPoint[] pathOptions, Entity entityIn, PathPoint currentPoint, PathPoint targetPoint, float maxDistance) {
        ...
        for (EnumFacing enumfacing : EnumFacing.values()) {
            ...
            if (pathpoint != null && !pathpoint.visited && pathpoint.distanceTo(targetPoint) < maxDistance) pathOptions[i++] = pathpoint;
        }
        ...
        for (int x = startX; x < startX + entitySizeX; ++x) {
            for (int y = startY; y < startY + entitySizeY; ++y) {
                for (int z = startZ; z < startZ + entitySizeZ; ++z) {
                    if (!BlockUtil.isFree(x, y, z, Chime.MC.theWorld != null ? Chime.MC.theWorld : blockaccess)) {
                        return null;
                    }
                }
            }
